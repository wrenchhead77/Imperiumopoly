...
public class CardManager : MonoBehaviour
{
    ...
    public void ExecuteCardAction(soCard cardData, iPlayer _player)
    {
        ...
        {
            case "PayAll50":
                foreach (var playerEntry in pm.players)
                {
                    iPlayer player = playerEntry.Value; // Access the Value of the KeyValuePair
                    if (player != currentPlayer && player.so_PlayerType.playerType != ePlayerType.none)
                    {
